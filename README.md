...
Главная особенность данной реализации — использование библиотеки компьютерного зрения **OpenCV.js** (WebAssembly версия) для выполнения вычислений.
...
├── index.html       # Единый файл (HTML разметка, CSS стили и JS логика)
└── README.md        # Описание проекта
Как запустить
Скачайте файл index.html.
Откройте его в любом современном браузере.
Важно: Для работы требуется подключение к интернету, так как библиотека OpenCV загружается через CDN (docs.opencv.org).
Пример использования
Дождитесь активации кнопки «Построить» (произойдет после загрузки OpenCV).
Выберите алгоритм, например «Сглаженная линия (OpenCV AA)».
Задайте координаты начала и конца отрезка.
Нажмите «Построить».
Наблюдайте, как пиксели разной прозрачности формируют сглаженную линию на сетке.
